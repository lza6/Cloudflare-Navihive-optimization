...
import { EnhancedNavigationClient } from './API/client';
import { MockNavigationClient } from './API/mock';
import { Site, Group, LoginResponse } from './API/http';
...
import { ThemeProvider } from '@mui/material/styles'; // 添加此行以避免命名冲突
...
const api =
  isDevEnvironment && !useRealApi
    ...
    : new EnhancedNavigationClient(isDevEnvironment ? 'http://localhost:8788/api' : '/api');
...
const MemoizedSortableGroupItem = memo(SortableGroupItem);
...
function App() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      setGroups(groupsWithSites.map(group => ({
        ...group,
        id: group.id || 0, // 确保id不为undefined
      })));
    } catch (error) {
  ...
  // 登录功能
  const handleLogin = useCallback(async (username: string, password: string, rememberMe: boolean = false) => {
    try {
      ...
      try {
        // 尝试按新API格式调用
        loginResponse = await (api as any).login({ username, password, rememberMe });
      } catch (error) {
        // 如果失败，尝试按旧API格式调用
        loginResponse = await (api as any).login(username, password, rememberMe);
      }
  ...
  }, [fetchData, fetchConfigs, handleError]);
  ...
  const renderLoginForm = useCallback(() => {
    return (
      ...
      >
        <MemoizedLoginForm onLogin={handleLogin} loading={loginLoading} error={loginError} />
      </Box>
    );
  }, [handleLogin, loginLoading, loginError]);
  ...
  const handleLogout = useCallback(async () => {
    ...
    setSnackbarOpen(true);
  }, [fetchData, fetchConfigs, handleMenuClose]);
  ...
  useEffect(() => {
    if (!loading && groups.length > 0) {
      ...
      if (process.env.NODE_ENV === 'production') {
        // 可以发送到分析服务
        // analytics.track('PageLoadTime', { loadTime });
      }
    }
  }, [loading, groups, loadStartTime]);
  ...
  // 保存分组排序
  const handleSaveGroupOrder = useCallback(async () => {
    try {
  ...
  const handleOpenAddGroup = useCallback(() => {
    setNewGroup(prev => ({ name: '', order_num: groups.length, is_public: 1 })); // 默认公开
  ...
  }, [newGroup, fetchData, handleCloseAddGroup, handleError]);
  ...
  const handleSiteInputChange = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    setNewSite(prev => ({
      ...prev,
      [e.target.name]: e.target.value,
    }));
  ...
  }, [newSite, fetchData, handleCloseAddSite, handleError]);

  // 配置相关函数
  const handleOpenConfig = useCallback(() => {
    setTempConfigs(prev => ({ ...configs }));
    setOpenConfig(true);
  }, [configs]);
  ...
  const handleConfigInputChange = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    setTempConfigs(prev => ({
      ...prev,
      [e.target.name]: e.target.value,
    }));
  ...
  const handleSaveConfig = useCallback(async () => {
    try {
      ...
      // 更新配置状态
      setConfigs(prev => ({ ...tempConfigs }));
      handleCloseConfig();
    } catch (error) {
      console.error('保存配置失败:', error);
      handleError('保存配置失败: ' + (error as Error).message);
    }
  }, [tempConfigs, configs, handleCloseConfig, handleError]);
  ...
  }, [importFile, fetchData, fetchConfigs, handleCloseImport, handleError]);
  ...
  // Memoize group data for performance
  ...
  const memoizedSitesData = useMemo(() =>
    groups.flatMap(g => g.sites || []), [groups]
  );
      ...
      >
        ...
        >
          ...
          {(() => {
            ...
            return (
              <Box sx={{ mb: 4 }}>
                <Paper elevation={2} sx={{ borderRadius: 3, overflow: 'hidden', p: 0.5, background: 'linear-gradient(45deg, #2D6CDF, #61DAFB)' }}>
                  <Box sx={{ bgcolor: 'background.paper', borderRadius: '14px', p: 1 }}>
                    <MemoizedSearchBox
                      groups={memoizedGroupData}
                      sites={memoizedSitesData}
          ...
          {/* 骨架屏加载效果 - 仅在首次加载时显示 */}
          {!loading && memoizedGroups.length === 0 && (
            <Box sx={{ py: 4 }}>
              <Skeleton variant="rectangular" width="60%" height={50} sx={{ mb: 4 }} />
              {[1, 2, 3].map((i) => (
                <Box key={i} sx={{ mb: 5 }}>
          ...
          {!loading && !error && (
            ...
            >
              {/* 数据统计面板 */}
              <MemoizedStatsPanel groups={memoizedGroupData} sites={memoizedSitesData} />

              {/* 高级数据分析 */}
              <MemoizedAdvancedAnalytics groups={memoizedGroupData} sites={memoizedSitesData} />
              ...
              {sortMode === SortMode.GroupSort ? (
                ...
                >
                  <SortableContext
                    items={memoizedGroups.map((group) => group.id.toString())}
                  ...
                  >
                    ...
                    >
                      {memoizedGroups.map((group) => (
                        <MemoizedSortableGroupItem key={group.id} id={group.id.toString()} group={group} />
                      ))}
                    </Stack>
                  </SortableContext>
                </DndContext>
        ...
        >
          ...
          >
            <Typography variant="body2" color="textSecondary">
              © {new Date().getFullYear()} NaviHive 导航站 - 由{' '}
              <a
                href="https://github.com/zqq-nuli/Cloudflare-Navihive"
                target="_blank"
            ...
            <FormControlLabel
              control={
                <Switch
                  ...
                  onChange={(e) =>
                    setNewSite(prev => ({ ...prev, is_public: e.target.checked ? 1 : 0 }))
                  }
                  color='primary'
                />
              }
